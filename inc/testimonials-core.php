...
// Always ensure this runs regardless of other code
add_action('init', 'portfolio_testimonials_register_directly', -999);

/**
 * Direct registration function with highest priority
 * This ensures the post type is registered even if the main function fails
 */
function portfolio_testimonials_register_directly() {
...
if (!function_exists('portfolio_testimonials_register_core')) {
     ...
     * This is the main registration function, but we also have the direct function above
     * as a fallback to ensure the post type always gets registered
     */
    function portfolio_testimonials_register_core() {
        // Add an indicator that this function was called
        update_option('portfolio_testimonials_core_registered', true);
        $labels = array(
            'name'                  => _x('Testimonials', 'Post type general name', 'portfolio'),
            'singular_name'         => _x('Testimonial', 'Post type singular name', 'portfolio'),
            'menu_name'             => _x('Testimonials', 'Admin Menu text', 'portfolio'),
